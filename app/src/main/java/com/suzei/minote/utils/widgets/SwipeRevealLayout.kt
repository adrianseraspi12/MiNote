...
class SwipeRevealLayout : ViewGroup {
    ...
    override fun onRestoreInstanceState(pState: Parcelable) {
        var state: Parcelable? = pState
     ...
     * {@inheritDoc}
     */
    override fun onMeasure(pWidthMeasureSpec: Int, pHeightMeasureSpec: Int) {
        var widthMeasureSpec = pWidthMeasureSpec
        var heightMeasureSpec = pHeightMeasureSpec
        if (childCount < 2) {
    ...
    fun open(animation: Boolean) {
        ...
        if (animation) {
            //  TODO: Fixed in the meantime
            mDragHelper!!.smoothSlideViewTo(mMainView!!, mRectMainOpen.left + resources.getDimension(R.dimen.margin_mid).toInt(), mRectMainOpen.top)
        } else {
            mDragHelper!!.abort()
            mMainView!!.layout(
                    mRectMainOpen.left,
    ...
    private val mainOpenLeft: Int
        get() = when (mDragEdge) {
            DRAG_EDGE_LEFT -> mRectMainClose.left + (mSecondaryView!!.width - resources.getDimension(R.dimen.margin_mid).toInt())
    ...
    private val mDragHelperCallback: ViewDragHelper.Callback = object : ViewDragHelper.Callback() {
        ...
        override fun clampViewPositionHorizontal(child: View, left: Int, dx: Int): Int {
            return when (mDragEdge) {
                DRAG_EDGE_RIGHT -> left.coerceAtMost(mRectMainClose.left).coerceAtLeast(mRectMainClose.left - mSecondaryView!!.width)
                DRAG_EDGE_LEFT -> left.coerceAtMost(mRectMainClose.left + mSecondaryView!!.width).coerceAtLeast(mRectMainClose.left)
                else -> child.left
            }
        }
